...
from utils.utils import print_attack_info, get_folder_name, get_slices, clip_by_tensor



import attacks.saa as saa
import attacks.vafa as vafa
...
from attacks.segpgd import seg_projected_gradient_descent_l_inf as segpgd_l_inf
    ...
    for i, batch in enumerate(val_loader):
        ...
        for start in range(0,len(slices),slice_batch_size):
            ...
            if args.attack_name=="pgd":
                ...
                DFB_Attack = saa.DFB(model, loss_fn, batch_size=images.shape[0], block_size=args.block_size, freq_band=args.freq_band, verbose=True)
                at_images, at_labels = DFB_Attack(images, labels)
            elif args.attack_name=="dfb-2d":
                DFB_Attack = saa.DFB_2D(model, loss_fn, batch_size=images.shape[0], block_size=args.block_size, freq_band=args.freq_band, verbose=True)
                at_images, at_labels = DFB_Attack(images, labels)
            elif args.attack_name=="cospgd":
                at_images = cospgd_l_inf(model, images, labels, loss_fn, n_classes=n_classes,  steps=args.steps, alpha=args.alpha, eps=args.eps/255.0, device=device, targeted=args.targeted, verbose=True)
            elif args.attack_name=="segpgd":
                at_images = segpgd_l_inf(model, images, labels, loss_fn, n_classes=n_classes,  steps=args.steps, alpha=args.alpha, eps=args.eps/255.0, device=device, targeted=args.targeted, verbose=True)            
            ...
            elif args.attack_name == "saa-2d":
                SP_Attack = saa.SAA_2D(model, loss_fn, batch_size=images.shape[0], block_size=args.block_size, steps=args.steps, targeted=args.targeted, rho=args.rho, lambda_dice=args.lambda_dice, use_ssim_loss=args.use_ssim_loss, lambda_ssim=args.lambda_ssim, freq_band=args.freq_band, verbose=True)    
                at_images, at_labels, perturbation = SP_Attack(images, labels)
            elif args.attack_name == "saa":
                SP_Attack = saa.SAA(model, loss_fn, batch_size=images.shape[0], block_size=args.block_size, steps=args.steps, n_classes=n_classes, targeted=args.targeted, rho=args.rho, lambda_dice=args.lambda_dice, use_ssim_loss=args.use_ssim_loss, lambda_ssim=args.lambda_ssim, freq_band=args.freq_band, verbose=True)    
                at_images, at_labels, perturbation = SP_Attack(images, labels)
            else:
        ...
        # images_hat = apply_defense(adv_val_inputs, "saa-d", device, block_size=(32,32,32))
        # adv_val_inputs = images_hat
        ...
        # img_adv.to_filename("/home/asif.hanif/adv_"+img_name)
