...
def print_attack_info(args):
    ...
    if args.attack_name=="cospgd":        print(f"\n CosPGD: alpha={args.alpha} , eps={args.eps} , steps={args.steps} , targeted={args.targeted}\n")
    if args.attack_name=="segpgd":        print(f"\n SegPGD: alpha={args.alpha} , eps={args.eps} , steps={args.steps} , targeted={args.targeted}\n")
    ...
    if args.attack_name=="saa":           print(f"\n SAA:  Rho={args.rho}, Steps={args.steps}, PatchSize={tuple(args.block_size)}, Freq_Band={args.freq_band}, LambdaDICE={args.lambda_dice}, UseSSIM={args.use_ssim_loss},  {'Lambda_SSIM='+str(args.lambda_ssim) if args.use_ssim_loss else ''},  Freq_Reg={args.freq_reg_mode}\n")
    if args.attack_name=="saa-2d":        print(f"\n SAA_2D:  Rho={args.rho}, Steps={args.steps}, PatchSize={tuple(args.block_size)}, Freq_Band={args.freq_band}, LambdaDICE={args.lambda_dice}, UseSSIM={args.use_ssim_loss},  {'Lambda_SSIM='+str(args.lambda_ssim) if args.use_ssim_loss else ''},  Freq_Reg={args.freq_reg_mode}\n")
...
def get_folder_name(args):
    ...
    elif args.attack_name == "segpgd" : folder_name = f"cospgd_alpha_{args.alpha}_eps_{args.eps}_i_{args.steps}"
    ...
    elif args.attack_name == "saa"    : folder_name = f"saa_rho_{args.rho}_i_{args.steps}_3d_dct_{args.block_size[0]}x{args.block_size[1]}x{args.block_size[2]}_lambda_dice_{args.lambda_dice}{'_with_ssim_lambda_'+str(args.lambda_ssim) if args.use_ssim_loss else ''}{'_freq_reg' if args.freq_reg_mode else ''}"
    elif args.attack_name == "saa-2d" : folder_name = f"saa_rho_{args.rho}_i_{args.steps}_2d_dct_{args.block_size[0]}x{args.block_size[1]}_lambda_dice_{args.lambda_dice}{'_with_ssim_lambda_'+str(args.lambda_ssim) if args.use_ssim_loss else ''}{'_freq_reg' if args.freq_reg_mode else ''}"
