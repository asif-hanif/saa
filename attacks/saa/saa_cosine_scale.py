...
class SAA(Attack):
    ...
    def __init__(self, model, loss_fn, batch_size=1, channels=1, height=96, width=96, depth=96, n_classes=None, steps=10, rho=0.2, block_size=(8,8,8), lambda_dice=1, use_ssim_loss=False, lambda_ssim=1, use_cosine_loss_scaling=True, targeted=False, freq_band='all', print_every=10, verbose=True):
        super(SAA, self).__init__("SAA", model)
        self.verbose = verbose

        assert n_classes is not None, f"Number of classes '{n_classes}' cannot be None. Specify the number of classes."
        ...
        self.use_cosine_loss_scaling = use_cosine_loss_scaling
    ...
    def forward(self, images, labels):
        ...
        if self.verbose and (images.max()>1 or images.min()<0) : warnings.warn(f"SAA Attack: Image values are expected to be in the range of [0,1], instead found [min,max]=[{images.min().item()} , {images.max().item()}]")
        ...
        for i in range(self.steps):
            ...
            if self.verbose and i==0: print(f"SAA: Perturbing '{self.freq_band}' frequency band(s)")
            ...
            #     if self.verbose and i==0: print('SAA: Using Targeted Attack ...\n')
            ...
            if self.use_ssim_loss:
                if self.verbose and i==0: print(f'SAA: Using SSIM Loss (LambdaSSIM={self.lambda_ssim})  ...')
                ssim_loss = ssim_loss_fn(images, adv_images)
                # ssim_loss = ssim_loss_fn(images, adv_images, patch_size=self.block_size); if self.verbose and i==0: print(f'SAA: Using Block-wise SSIM Loss (LambdaSSIM={self.lambda_ssim})  ...')


            
            if self.use_cosine_loss_scaling:
                if self.verbose and i==0: print(f'SAA: Using Cosine Loss Scaling  ...')
                labels_onehot = one_hot(labels, num_classes=self.n_classes) # [B,1,H,W,D] -->  [B,NumClass,H,W,D]
                pred_softmax  = self.softmax(logits)                        # [B,NumClass,H,W,D]
                cos_sim = self.cosine_similarity(pred_softmax, labels_onehot).mean()
            
            
            # calculate loss
            if self.targeted:
                if self.verbose and i==0: print('SAA: Using Targeted Attack ...\n')
                loss = (self.lambda_dice*dice_loss*(1-cos_sim)*1.5 if self.use_cosine_loss_scaling else self.lambda_dice*dice_loss) + (self.lambda_ssim*ssim_loss if self.use_ssim_loss else 0.0) 
            else:
                # loss = -1*(self.lambda_dice*dice_loss*cos_sim*1.5 if self.use_cosine_loss_scaling else self.lambda_dice*dice_loss) + (self.lambda_ssim*ssim_loss if self.use_ssim_loss else 0.0) 
                loss = -1*(self.lambda_dice*dice_loss+(1-cos_sim)*5.5 if self.use_cosine_loss_scaling else self.lambda_dice*dice_loss) + (self.lambda_ssim*ssim_loss if self.use_ssim_loss else 0.0) 
                # loss = self.lambda_dice*dice_loss + (self.lambda_ssim*ssim_loss if self.use_ssim_loss else 0.0) 
            ...
            if self.verbose and (i==0 or (i+1)%self.print_every == 0): print(f"Step: {str(i+1).zfill(2)}    Dice_Loss: {dice_loss.item():0.5f}    {'SSIM_Loss: '+str(round(ssim_loss.item(),5)) if self.use_ssim_loss else ''}   {'CosSim: '+str(round(cos_sim.item(),5)) if self.use_cosine_loss_scaling else ''}   Total_Loss: {loss.item():0.5f}   Success_Rate: {round(success_rate*100,2)} (%)")     
...
class SAA_2D(Attack):
    ...
    def __init__(self, model, loss_fn, batch_size=1, channels=1, height=96, width=96, depth=96, steps=10, rho=0.2, block_size=(8,8), lambda_dice=1, use_ssim_loss=False, lambda_ssim=1, targeted=False, freq_band='all', print_every=10, verbose=True):
        super(SAA_2D, self).__init__("SAA_2D", model)
        self.verbose = verbose
    ...
    def forward(self, images, labels):
        ...
        for i in range(self.steps):
            ...
            if self.verbose and i==0: print(f"SAA-2D: Perturbing '{self.freq_band}' frequency band(s)")
            if self.freq_band == 'all':
            ...
            if self.targeted: # for targetted attack
                if self.verbose and i==0: print('SAA-2D: Using Targeted Attack ...\n')
                dice_loss  = self.loss_fn(logits, labels)
            else: # for un-targetted attack
                dice_loss  = -1*self.loss_fn(logits, labels)
            ...
            if self.use_ssim_loss:
                if self.verbose and i==0: print(f'SAA-2D: Using SSIM Loss (LambdaSSIM={self.lambda_ssim})  ...')
                ssim_loss = ssim_loss_fn(images, adv_images)
...
class SAA_2D_ForClassification(Attack):
    r"""  
    Arguments:
        - model (nn.Module): model to attack.
        - steps (int): number of steps. (DEFALUT: 20)
        ...
        - targeted: True for targeted attack
    Shapes:
        - images: [B,C,H,W] normalized to [0,1]. B = BatchSize, C = Number of Channels,  H = Height,  W = Width
        - labels: [B]
    """

    def __init__(self, model, loss_fn, batch_size=1, channels=3, height=224, width=224, depth=1, steps=10, rho=0.2, block_size=(8,8), lambda_ce=1, use_ssim_loss=False, lambda_ssim=1, targeted=False, freq_band='all', print_every=10, verbose=True):
        super(SAA_2D_ForClassification, self).__init__("SAA_2D_ForClaasification", model)
        self.verbose = verbose
        ...
        self.depth  = depth
        assert self.depth==1 , f"Set 'depth=1' for SAA on natural images."
    ...
    def forward(self, images, labels):
        ...
        labels   = labels.clone().detach().to(self.device)       #  [B]
        ...
        for i in range(self.steps):
            ...
            images = images.unsqueeze(2)                              #  [B,C,H,W] --> [B,C,D=1,H,W]

            blocks = block_splitting_2d(images, self.block_size)      # [B, C, D=1, H, W] --> [B, C, D=1, N_Blocks, Block_H, Block_W]
            blocks_dct = dct_2d(blocks)                               # [B, C, D=1, N_Blocks, Block_H, Block_W] , 2D DCT is computed on last 2 dimensions  
            ...
            blocks_idct = idct_2d(blocks_perturbed_dct)                   # [B, C, D=1, N_Blocks, Block_H, Block_W] , 2D IDCT is computed on last 2 dimensions 
            merged_blocks = block_merging_2d(blocks_idct, images.shape)   # [B, C, D=1, N_Blocks, Block_H, Block_W] --> [B,C,D=1,H,W]


            # adversarially perturbed images
            adv_images = merged_blocks # [B,C,D=1,H,W]
            adv_images = adv_images.squeeze(2) # [B,C,D=1,H,W] -->  [B,C,H,W]

            images = images.squeeze(2) # [B,C,D=1,H,W] -->  [B,C,H,W]

            logits = self.model(adv_images) # logits: [B,NumClasses] , passing adversarial images through the model 
            ...
            if self.targeted: # for targetted attack
                ...
                ce_loss  = -1*self.loss_fn(logits, labels)


            if self.use_ssim_loss:
                if self.verbose and i==0: print(f'SAA-2D: Using SSIM Loss (LambdaSSIM={self.lambda_ssim})  ...')
                ssim_loss = ssim_loss_fn_2d_images(images, adv_images)



            total_loss = self.lambda_ce*ce_loss + (self.lambda_ssim*ssim_loss if self.use_ssim_loss else 0.0) 
            optimizer.zero_grad()
            total_loss.backward()
            ...
            # print losses and success rate
            if self.verbose and (i==0 or (i+1)%self.print_every == 0): print(f"Step: {str(i+1).zfill(2)}    CE_Loss: {ce_loss.item():0.5f}    {'SSIM_Loss: '+str(round(ssim_loss.item(),5)) if self.use_ssim_loss else ''}   Total_Loss: {total_loss.item():0.5f}  Success_Rate: {round(success_rate*100,2)} (%)")     


            if success_rate >= 1:
                print(f"\nEnding at Step={i+1} with SuccessRate={success_rate*100:0.2f} (%)")
                adv_images = torch.clamp(adv_images, min=0, max=1.0).detach()
                return adv_images, pred_labels, self.perturbation.detach()       
