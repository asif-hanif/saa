...
class SAA(Attack):
    ...
    def __init__(self, model, loss_fn, batch_size=1, channels=1, height=96, width=96, depth=96, n_classes=None, steps=10, rho=0.2, block_size=(8,8,8), lambda_dice=1, use_ssim_loss=False, lambda_ssim=1, targeted=False, freq_band='all', print_every=10, verbose=True):
    ...
    def forward(self, images, labels):
        ...
        for i in range(self.steps):
            ...
            if self.targeted:
                ...
                loss = -1*(self.lambda_dice*dice_loss) + (self.lambda_ssim*ssim_loss if self.use_ssim_loss else 0.0) 
            ...
            if self.verbose and (i==0 or (i+1)%self.print_every == 0): print(f"Step: {str(i+1).zfill(2)}    Dice_Loss: {dice_loss.item():0.5f}    {'SSIM_Loss: '+str(round(ssim_loss.item(),5)) if self.use_ssim_loss else ''}   Total_Loss: {loss.item():0.5f}   Success_Rate: {round(success_rate*100,2)} (%)")     
