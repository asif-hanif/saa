...
class VAFA(Attack):
    ...
    def forward(self, images, labels):
        ...
        for i in range(self.steps):
            ...
            adv_images = (adv_images - minn + 1e-6)/(maxx-minn + 1e-6)
...
#############################################################################################################
#                                            InfoDrop 3D (Under Testing)                                    #
#############################################################################################################
# Testing Purpose

class InfoDrop3D_New(Attack):
    r"""  
    Arguments:
        - model (nn.Module): model to attack.
        - steps (int): number of steps. (DEFALUT: 20)
    ...
    def __init__(self, model,loss_fn, batch_size=1, channels=1, height=96, width=96, depth=96, steps=20, q_min=1, q_max=10, block_size=(8,8,8), use_ssim_loss=False, targeted=False, verbose=True):
        super(InfoDrop3D_New, self).__init__("InfoDrop3D_New", model)
        self.verbose = verbose
        ...
        num_blocks = (self.height*self.width*self.depth)//(np.prod(self.block_size))
        q_tables_shape = (self.batch_size,self.channels,num_blocks)+self.block_size    # [B, C, N_Blocks, Block_H, Block_W, Block_D]
        self.q_tables = torch.full( q_tables_shape, q_min, dtype = torch.float32)      # initialize the q_tables with q_min
        
        # learnable quantization table values will be limited to a certain range 
        # upper bound on q_tables values will gradually increase in each iteration 
        self.q_range = np.linspace(q_min, q_max, steps+1)
    ...
    def forward(self, images, labels):
        ...
        for i in range(self.steps):
            ...
            # update qunatization tables
            self.q_tables = self.q_tables.detach() - torch.sign(self.q_tables.grad)
            self.q_tables = torch.clamp(self.q_tables, self.q_range[0], self.q_range[i+1]).detach()
        ...
        ## if ssim_loss exceeds a threshold, replace adv_images with clean images
        # if self.use_ssim_loss and ssim_loss > 0.45: adv_images = images.detach()
